...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.n_class_primary = 2
        self.n_channels = configs.n_channels
        self.n_kernel_t = 8
        self.n_kernel_s = 16
        self.dropout = 0.5
        self.kernel_length = 16

        self.block_shared_feature_extractor = nn.Sequential(
            # 原block1
            nn.ZeroPad2d((7, 8, 0, 0)),
            nn.Conv2d(1, 8, (1, 16), bias=False),
            nn.BatchNorm2d(8),
            # 原block2
            nn.Conv2d(8, 16, (self.n_channels, 1), groups=8, bias=False),
            nn.BatchNorm2d(16),
            nn.ELU(),
            nn.AvgPool2d((1, 2)),
        ...
        self.primary_task_classifier = nn.Sequential(
            nn.Linear(self.n_kernel_s * 128 // 16, self.n_class_primary)
        )
        self.vto_task_classifier = nn.Sequential(
            nn.Linear(self.n_kernel_s * 128 // 16, 9)
        )
        self.msp_task_classifier = nn.Sequential(
            nn.Linear(self.n_kernel_s * 128 // 16, 8)
        )
