...
class CAFormerTrainer(ExpBasic):
    ...
    def _encode_augmented_views(self, batch_x, eeg_augmentation):
        return self._augment_data(batch_x, eeg_augmentation)
    ...
    def validate(self, vali_loader, criterion, criterion_c, augmentor, contrastive_loss_weight=0.5):
        self.model.eval()
        ...
        with torch.no_grad():
            for batch_x, label in vali_loader:
                ...
                features_aug = self._encode_augmented_views(batch_x, augmentor)
            ...
            loss = ce_loss + sc_loss * contrastive_loss_weight
    ...
    def train(self, setting, fold_i, train_val_set, verbose=True):
        contrastive_loss = SupervisedContrastiveLoss(
            temperature=self.args.temperature_tau,
            base_temperature=self.args.temperature_tau,
        )
        eeg_augmentation = EEGAugmentation(
            self.args.dataset,
            getattr(self.args, "augmentation_profile", "dataset_default"),
        )
        ...
        logging.info(
            f"{self.args.sub_name} fold{fold_i + 1} CSSL augmentation profile: "
            f"{eeg_augmentation.profile_name}"
        )
        logging.info(
            f"{self.args.sub_name} fold{fold_i + 1} CSSL augmentations: "
            f"{', '.join(eeg_augmentation.active_augmentation_names())}"
        )
        logging.info(
            f"{self.args.sub_name} fold{fold_i + 1} CSSL hyperparameters: "
            f"gamma={self.args.contrastive_loss_weight}, tau={self.args.temperature_tau}"
        )
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for batch_x, label in train_loader:
                ...
                batch_x, label = batch_x.to(self.device), label.to(self.device)
                outputs, _ = self.model(batch_x)
                features_aug = self._encode_augmented_views(batch_x, eeg_augmentation)
                ce_loss = criterion(outputs, label.long())
                sup_loss = contrastive_loss(features_aug)
                loss = ce_loss + sup_loss * contrastive_loss_weight
            ...
            val_loss, val_ce_loss, val_c_loss, val_auc, val_ba, val_tpr, val_tnr = self.validate(
                val_loader, criterion, contrastive_loss, eeg_augmentation, contrastive_loss_weight
            )
...
SupConLoss = SupervisedContrastiveLoss


class EEGAugmentation:
    """Dataset-aware EEG augmentations used to build contrastive pairs.

    Historical experiments used different augmentation pairs by dataset:
    - THU/CAS: sign flip + controlled additive temporal noise
    - GIST: channel dropout + amplitude scaling
    """
    ...
    PROFILE_TO_AUGMENTATIONS = {
        "thu_cas_default": ("signflip", "add_noise"),
        "gist_default": ("channel_dropout", "amplitude_scaling"),
        "signflip_only": ("identity", "signflip"),
        "add_noise_only": ("identity", "add_noise"),
        "channel_dropout_only": ("identity", "channel_dropout"),
        "amplitude_scaling_only": ("identity", "amplitude_scaling"),
        "thu_cas_alternative": ("channel_dropout", "amplitude_scaling"),
        "gist_alternative": ("signflip", "add_noise"),
    }

    def __init__(self, dataset_name="THU", augmentation_profile="dataset_default"):
        self.dataset_name = dataset_name
        self.profile_name = self._resolve_profile_name(augmentation_profile)

    def _resolve_profile_name(self, augmentation_profile):
        if augmentation_profile in ("", None, "dataset_default"):
            if self.dataset_name not in self.DEFAULT_PROFILE_BY_DATASET:
                raise ValueError(f"Unsupported dataset for default augmentation profile: {self.dataset_name}")
            return self.DEFAULT_PROFILE_BY_DATASET[self.dataset_name]

        if augmentation_profile not in self.PROFILE_TO_AUGMENTATIONS:
            supported_profiles = ", ".join(sorted(self.PROFILE_TO_AUGMENTATIONS))
            raise ValueError(
                f"Unsupported augmentation_profile='{augmentation_profile}'. "
                f"Available profiles: {supported_profiles}"
            )
    ...
    def active_augmentations(self):
        augmentations = []
        for augmentation_name in self.PROFILE_TO_AUGMENTATIONS[self.profile_name]:
            augmentations.append((augmentation_name, getattr(self, augmentation_name)))
        return augmentations

    def active_augmentation_names(self):
        return [name for name, _ in self.active_augmentations()]

    def augment(self, data):
        return [augmentation_fn(data.clone()) for _, augmentation_fn in self.active_augmentations()]


DataAugmentation = EEGAugmentation
RsvpClassificationWithCSSL = CAFormerTrainer
