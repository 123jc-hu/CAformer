...
from Data_Processing.make_dataset import SupportQuerySplit, dataloader
...
class PrototypeClassification(ExpBasic):
    def __init__(self, args):
        super(PrototypeClassification, self).__init__(args)
    ...
    def compute_prototypes(self, support_features, Ns) -> torch.Tensor:
        """Compute class prototypes from support samples."""
        class_prototypes = support_features.reshape(2, Ns, -1).mean(dim=1)
        return class_prototypes

    def cal_distances(self, query_vectors, prototype_vectors, mode="inner_product") -> torch.Tensor:
        """
    ...
    def validate(self, val_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, label) in enumerate(val_loader):
                ...
                support_embeddings = embeddings[:2 * self.args.val_Ns]
                support_label = label[:2 * self.args.val_Ns]
                query_embeddings = embeddings[2 * self.args.val_Ns:]
                query_label = label[2 * self.args.val_Ns:]

                prototypes = self.compute_prototypes(support_embeddings, self.args.val_Ns)
    ...
    def train(self, setting, fold_i, train_val_set, verbose=True):
        ...
        # 对训练集划分支持集和查询集
        support_query_model = SupportQuerySplit(train_x, train_y, self.args.episodes, self.args.train_Ns,
                                                self.args.train_Nq)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label) in enumerate(train_loader):
                ...
                # Embed all samples
                embeddings = self.model(batch_x)
                support_embeddings = embeddings[:2*self.args.train_Ns]
                support_label = label[:2*self.args.train_Ns]
                query_embeddings = embeddings[2*self.args.train_Ns:]
                query_label = label[2*self.args.train_Ns:]

                prototypes = self.compute_prototypes(support_embeddings, self.args.train_Ns)
    ...
    def test(self, setting, fold_i, train_val_set, test_set, result_path, test=0):
        ...
        if self.args.n_shot == 0:
            test_Ns = self.args.train_Ns
            support_model = SupportQuerySplit(
            ...
            support_samples = support_set_x[:2*self.args.train_Ns]
        else:
            test_Ns = self.args.n_shot
            support_model = SupportQuerySplit(test_x, test_y, 1, test_Ns, None)
            support_set_x, support_set_y, _ = support_model.support_query_set_split(mode="validate")
        ...
        with torch.no_grad():
            for i, (batch_x, label) in enumerate(test_loader):
                ...
                support_embeddings = self.model(support_samples.to(self.device))
                prototypes = self.compute_prototypes(support_embeddings, test_Ns)
                query_embeddings = self.model(batch_x)
                distances = self.cal_distances(query_embeddings, prototypes, mode="l2")  # (Nq, 2)
